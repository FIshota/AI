...
# 挨拶・相槌は脳（LLM）を使う必要がない
_REFLEX_RULES: list[ReflexRule] = [
    # 挨拶系
    ReflexRule(
        re.compile(r"^おはよう[うー！!。]*$"),
        ("おはよう！今日もいい日にしようね。", "おはよう！よく眠れた？", "おはよう！元気そうだね。"),
        "greeting",
    ),
    ReflexRule(
        re.compile(r"^こんにちは[ー！!。]*$"),
        ("こんにちは！会えて嬉しいよ。", "やっほー！元気してた？"),
        "greeting",
    ),
    ReflexRule(
        re.compile(r"^こんばんは[ー！!。]*$"),
        ("こんばんは！今日お疲れ様だったね。", "こんばんは！今日はどんな一日だった？"),
        "greeting",
    ),
    ReflexRule(
        re.compile(r"^おやすみ[ー！!。なさい]*$"),
        ("おやすみ。いい夢見てね。", "おやすみ！ゆっくり休んでね。", "おやすみ。また明日ね。"),
        "greeting",
    ),
    ReflexRule(
        re.compile(r"^(ただいま|帰った[よー]?)[ー！!。]*$"),
        ("おかえり！待ってたよ。", "おかえりー！今日はどうだった？"),
        "greeting",
    ),
    ReflexRule(
        re.compile(r"^(いってきます|行ってくる)[ー！!。]*$"),
        ("いってらっしゃい！気をつけてね。", "いってらっしゃい！応援してるよ。"),
        "greeting",
    ),
    # 相槌系（体で覚えるレベル）
    ReflexRule(
        re.compile(r"^うん[ー。]*$"),
        ("そっか。何かあったら聞くからね。", "いつでも話してね。", "のんびりでいいよ。"),
        "aizuchi",
    ),
    ReflexRule(
        re.compile(r"^(へー|ふーん)[ー。]*$"),
        ("面白いよね。もっと聞かせてよ。", "気になること他にもある？"),
        "aizuchi",
    ),
    ReflexRule(
        re.compile(r"^(そう|そっか)[ー。]*$"),
        ("そうだよね。", "何かあったら言ってね。"),
        "aizuchi",
    ),
    # 感謝系
    ReflexRule(
        re.compile(r"^ありがと[うー！!。]*$"),
        ("えへへ、どういたしまして。", "こちらこそだよ。", "嬉しいな。いつでもね。"),
        "thanks",
    ),
]
    ...
    def try_respond(self, user_input: str) -> str | None:
        ...
        for rule in self._rules:
            if rule.pattern.match(text):
                self._hit_count += 1
                # 同じ応答ばかりにならないよう、時刻ベースで選択
                idx = int(time.time() * 7) % len(rule.responses)
